<template>
  <div>
    <div class="d-flex justify-content-between align-items-center px-2 py-2">
      <h1 class="border-bottom-0 mb-0">Kayıt Düzenle</h1>
    </div>
    <hr>
...
export default {
  ...
  mounted() {
    this.model = this.getEntryById(parseInt(this.id, 10));
  },
  ...
  computed: {
    ...
    ...mapGetters('entries', ['getEntryById']),
  },
  methods: {
    ...mapActions('entries', {
    ...
    save() {
      this.$validator.validateAll().then(isValid => {
        if (isValid) {
          this.editEntryAction({ entry: this.model });
